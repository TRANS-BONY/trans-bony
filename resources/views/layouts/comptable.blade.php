...
<body x-data="comptableApp()" :class="{'dark': isDarkMode}" class="bg-gray-50 dark:bg-gray-900 text-gray-900 dark:text-white">

<div class="flex min-h-screen">

    <!-- ═══════════ SIDEBAR ═══════════ -->
    <div
        class="fixed inset-y-0 left-0 z-30 w-72 bg-gradient-to-br from-emerald-900 via-teal-900 to-cyan-900 dark:from-gray-800 dark:to-gray-900 shadow-2xl transform transition-all duration-300 ease-in-out lg:relative lg:translate-x-0 overflow-y-auto flex flex-col"
        ...
        <div class="p-6 flex-1">
            ...
            <div class="mb-8 p-4 rounded-xl bg-white/10 backdrop-blur-sm border border-white/10">
                <div class="flex items-center space-x-3">
                    <div class="w-10 h-10 rounded-full bg-gradient-to-br from-emerald-400 to-teal-500 flex items-center justify-center shadow-md flex-shrink-0">
                        <i class="fas fa-user-tie text-white text-sm"></i>
                    </div>
                    <div class="min-w-0">
                        <p class="text-sm font-semibold text-white truncate">{{ auth()->user()->name }}</p>
                        <span class="inline-flex items-center gap-1 px-2 py-0.5 rounded-full bg-emerald-500/30 text-emerald-300 text-xs font-medium mt-0.5">
            ...
            <!-- Navigation -->
            <div class="space-y-1">
                <p class="text-xs uppercase tracking-widest text-emerald-400/70 mb-4 font-semibold px-1">
                    Navigation
                </p>
                ...
                <a href="{{ route('comptable.recettes.index') }}" class="nav-item flex items-center space-x-3 px-4 py-3 text-white/90 hover:text-white group nav-fluid-hover" x-data="{ hover: false }" @mouseenter="hover = true" @mouseleave="hover = false" :class="hover ? 'shadow-lg -translate-y-1 bg-white/10 scale-[1.02] floating-fast' : ''">
                    ...
                    @if(isset($sidebar_recettes) && $sidebar_recettes > 0)
                        <span class="text-xs bg-emerald-600/60 px-2 py-0.5 rounded-full font-semibold text-emerald-100">
                            {{ $sidebar_recettes }}
                        </span>
                    @endif
                </a>
    ...
    <div class="flex-1 flex flex-col min-h-screen overflow-hidden">
        ...
        <header class="bg-white dark:bg-gray-800 shadow-sm sticky top-0 z-10 border-b border-gray-100 dark:border-gray-700">
            <div class="px-4 sm:px-6 py-3 flex justify-between items-center">
                <div class="flex items-center space-x-4">
                    ...
                    <div>
                        <h1 class="text-lg sm:text-xl font-bold bg-gradient-to-r from-emerald-600 to-teal-600 bg-clip-text text-transparent" x-text="pageTitle">Tableau de bord</h1>
                    ...
                    <div class="relative" x-data="{ open: false }">
                        <button @click="open = !open" class="flex items-center gap-2 px-3 py-2 rounded-xl hover:bg-gray-100 dark:hover:bg-gray-700 transition" @mouseenter="$el.classList.add('floating-fast', 'scale-105')" @mouseleave="$el.classList.remove('floating-fast', 'scale-105')">
                            ...
                            <div class="hidden md:block text-left">
                                ...
                                <p class="text-xs text-gray-400 dark:text-gray-500">Comptable</p>
                            </div>
                        ...
                        <div x-show="open" @click.away="open = false" x-transition
                            ...
                            <div class="p-2">
                                <a href="{{ route('profile.edit') }}" class="flex items-center gap-3 px-4 py-2.5 text-sm text-gray-700 dark:text-gray-300 hover:bg-gray-50 dark:hover:bg-gray-700 rounded-xl transition">
                                    <i class="fas fa-user-circle text-emerald-500 w-4"></i> Mon profil
                                </a>
        ...
        <footer class="px-6 py-3 border-t border-gray-100 dark:border-gray-700 bg-white dark:bg-gray-800">
            <p class="text-xs text-gray-400 text-center">TRANS BONY &copy; {{ date('Y') }} — Espace Comptable</p>
        </footer>
    </div>
</div>

<script>
function comptableApp() {
    return {
        sidebarOpen: false,
        isDarkMode: false,
        ...
        init() {
            ...
            document.querySelectorAll('.nav-item').forEach(item => {
                ...
                if (href && ((href === '/comptable/dashboard' && path === href) || (href !== '/comptable/dashboard' && path.startsWith(href)))) {
