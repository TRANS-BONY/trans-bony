...
<body x-data="agentApp()" :class="{'dark': isDarkMode}" class="bg-gray-50 dark:bg-gray-900 text-gray-900 dark:text-white">

<div class="flex min-h-screen">

    <!-- ═══════════ SIDEBAR ═══════════ -->
    <div
        class="fixed inset-y-0 left-0 z-30 w-72 bg-gradient-to-br from-indigo-900 via-blue-900 to-sky-900 dark:from-gray-800 dark:to-gray-900 shadow-2xl transform transition-all duration-300 ease-in-out lg:relative lg:translate-x-0 overflow-y-auto flex flex-col"
        ...
        <div class="p-6 flex-1">
            ...
            <div class="flex items-center justify-between mb-10">
                <div class="flex items-center space-x-3">
                    <div class="w-11 h-11 rounded-xl bg-gradient-to-br from-blue-400 to-indigo-500 flex items-center justify-center shadow-lg">
                        <i class="fas fa-headset text-white text-lg"></i>
                    </div>
                    <div>
                        <h2 class="text-lg font-bold text-white">TRANS BONY</h2>
                        <p class="text-xs text-blue-300 font-medium">Espace Agent</p>
                    </div>
                </div>
            ...
            <div class="mb-8 p-4 rounded-xl bg-white/10 backdrop-blur-sm border border-white/10">
                <div class="flex items-center space-x-3">
                    <div class="w-10 h-10 rounded-full bg-gradient-to-br from-blue-400 to-indigo-500 flex items-center justify-center shadow-md flex-shrink-0">
                        <i class="fas fa-user text-white text-sm"></i>
                    </div>
                    <div class="min-w-0">
                        <p class="text-sm font-semibold text-white truncate">{{ auth()->user()->name }}</p>
                        <span class="inline-flex items-center gap-1 px-2 py-0.5 rounded-full bg-blue-500/30 text-blue-300 text-xs font-medium mt-0.5">
                            <span class="w-1.5 h-1.5 rounded-full bg-blue-400 animate-pulse-soft"></span>
                            Agent
                        </span>
                    </div>
                </div>
            ...
            <!-- Navigation -->
            <div class="space-y-1">
                <p class="text-xs uppercase tracking-widest text-blue-400/70 mb-4 font-semibold px-1">
                    Navigation
                </p>

                <!-- Dashboard -->
                <a href="{{ route('agent.dashboard') }}" class="nav-item flex items-center space-x-3 px-4 py-3 text-white/90 hover:text-white group nav-fluid-hover" x-data="{ hover: false }" @mouseenter="hover = true" @mouseleave="hover = false" :class="hover ? 'shadow-lg -translate-y-1 bg-white/10 scale-[1.02] floating-fast' : ''">
                    <div class="w-8 h-8 flex items-center justify-center rounded-lg bg-white/10 group-hover:bg-blue-500/30 transition-all">
                        <i class="fas fa-tachometer-alt text-sm text-blue-300"></i>
                    </div>
    ...
    <div class="flex-1 flex flex-col min-h-screen overflow-hidden">
        ...
        <header class="bg-white dark:bg-gray-800 shadow-sm sticky top-0 z-10 border-b border-gray-100 dark:border-gray-700">
            <div class="px-4 sm:px-6 py-3 flex justify-between items-center">
                <div class="flex items-center space-x-4">
                    <button @click="sidebarOpen = !sidebarOpen" class="lg:hidden text-gray-500 dark:text-gray-400 hover:text-blue-600 transition p-2 hover:bg-gray-100 dark:hover:bg-gray-700 rounded-xl" @mouseenter="$el.classList.add('floating-fast', 'scale-105')" @mouseleave="$el.classList.remove('floating-fast', 'scale-105')">
                        <i class="fas fa-bars text-lg"></i>
                    </button>
                    <div>
                        <h1 class="text-lg sm:text-xl font-bold bg-gradient-to-r from-blue-600 to-indigo-600 bg-clip-text text-transparent" x-text="pageTitle">Tableau de bord</h1>
                    ...
                    <div class="relative" x-data="{ open: false }">
                        <button @click="open = !open" class="flex items-center gap-2 px-3 py-2 rounded-xl hover:bg-gray-100 dark:hover:bg-gray-700 transition" @mouseenter="$el.classList.add('floating-fast', 'scale-105')" @mouseleave="$el.classList.remove('floating-fast', 'scale-105')">
                            ...
                            <div class="hidden md:block text-left">
                                ...
                                <p class="text-xs text-gray-400 dark:text-gray-500">Agent</p>
                            </div>
                        ...
                        <div x-show="open" @click.away="open = false" x-transition
                            ...
                            <div class="p-2">
                                <a href="{{ route('profile.edit') }}" class="flex items-center gap-3 px-4 py-2.5 text-sm text-gray-700 dark:text-gray-300 hover:bg-gray-50 dark:hover:bg-gray-700 rounded-xl transition">
                                    <i class="fas fa-user-circle text-blue-500 w-4"></i> Mon profil
                                </a>
        ...
        <footer class="px-6 py-3 border-t border-gray-100 dark:border-gray-700 bg-white dark:bg-gray-800">
            <p class="text-xs text-gray-400 text-center">TRANS BONY &copy; {{ date('Y') }} — Espace Agent</p>
        </footer>
    </div>
</div>
    ...
    return {
        ...
        init() {
            ...
            document.querySelectorAll('.nav-item').forEach(item => {
                ...
                if (href && ((href === '/agent/dashboard' && path === href) || (href !== '/agent/dashboard' && path.startsWith(href)))) {
