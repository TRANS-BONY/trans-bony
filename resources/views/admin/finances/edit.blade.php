...
<div class="space-y-6">
    ...
    <div class="relative overflow-hidden rounded-2xl bg-gradient-to-r from-emerald-600 via-teal-600 to-cyan-600 p-6 animate-slide-down shadow-xl">
        ...
        <div class="relative flex flex-col md:flex-row justify-between items-start md:items-center gap-4">
            <div class="flex items-center gap-4">
                ...
                <div>
                    <h1 class="text-2xl md:text-3xl font-bold text-white">Modifier Recette Mensuelle</h1>
                    <p class="text-emerald-100">Mois : {{ \Carbon\Carbon::parse($recette->mois)->format('F Y') }} | {{ number_format($recette->montant, 2) }} FCFA</p>
                </div>
            </div>
    ...
    <div class="bg-white rounded-2xl shadow-xl overflow-hidden animate-fade-in-up">
        <form method="POST" action="{{ route('admin.recettes.update', $recette) }}" class="p-8 space-y-8">
            @csrf @method('PUT')
            ...
            <div>
                ...
                <select name="vehicule_id" required class="w-full p-4 border border-gray-200 rounded-xl focus:ring-4 focus:ring-emerald-500 focus:border-emerald-500 bg-white shadow-inner transition-all duration-300 text-lg h-14 hover:border-emerald-400">
                    @foreach($vehicules as $vehicule)
                        <option value="{{ $vehicule->id }}" {{ $recette->vehicule_id == $vehicule->id ? 'selected' : '' }}>
            ...
            <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
                ...
                <div>
                    ...
                    <input type="date" name="date" required
                           value="{{ old('date', $recette->date?->format('Y-m-d')) }}"
                           class="w-full p-4 border border-gray-200 rounded-xl focus:ring-4 focus:ring-emerald-500 focus:border-emerald-500 bg-white shadow-inner transition-all duration-300 text-lg h-14 hover:border-emerald-400">
