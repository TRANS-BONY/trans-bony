...
<div class="module-index-wrapper custom-scrollbar">
    <div class="flex items-center justify-between bg-white dark:bg-gray-800 p-6 rounded-2xl shadow-sm border border-gray-100 dark:border-gray-700">
        <div>
            <h1 class="text-2xl font-bold text-gray-900 dark:text-white">Maintenances</h1>
            <p class="text-sm text-gray-500 dark:text-gray-400 mt-1">Suivi technique du parc</p>
        </div>
    </div>
            ...
            <table class="w-full text-left border-collapse whitespace-nowrap">
                ...
                <tbody class="divide-y divide-gray-100 dark:divide-gray-700 text-sm">
                    ...
                    <tr class="hover:bg-gray-50 dark:hover:bg-gray-700/50 transition">
                        <td class="p-4">
                            <p class="font-bold text-gray-900 dark:text-white">{{ $m->vehicule->immatriculation ?? 'N/A' }}</p>
                        </td>
                        <td class="p-4 text-gray-700 dark:text-gray-300">{{ ucfirst($m->type) }}</td>
                        <td class="p-4 text-gray-700 dark:text-gray-300">{{ $m->date_prevue->format('d/m/Y') }}</td>
                        <td class="p-4">
                            <span class="px-2 py-1 text-xs font-semibold rounded-lg bg-gray-100 text-gray-700">{{ ucfirst($m->statut) }}</span>
                        </td>
                        <td class="p-4 text-right">
                            <a href="{{ route('manager.maintenances.show', $m) }}" class="inline-flex items-center justify-center w-8 h-8 rounded-lg text-gray-400 hover:text-purple-600 hover:bg-purple-50 transition" title="Voir">
                                <i class="fas fa-eye"></i>
                            </a>
                        </td>
                    </tr>
                    @empty
                    <tr><td colspan="5" class="p-8 text-center text-gray-500">Aucune maintenance enregistrée.</td></tr>
                    @endforelse
                </tbody>
            </table>
        </div>
        @if($maintenances->hasPages())
        <div class="p-4 bg-gray-50 shrink-0">{{ $maintenances->links() }}</div>
        @endif
    </div>
</div>
@endsection
